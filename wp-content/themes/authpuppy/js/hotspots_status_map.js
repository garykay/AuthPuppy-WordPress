...
{
    // Init HTML
    var html = "<table><tr><td valign=\"top\"><img src='" + icon.image + "' /></td><td>";
    ...
    if (name.length == 1) {
		    if (websiteUrl.length == 1) {
		        html += "<strong><a href=\"" + GXml.value(websiteUrl[0]) + "\" target=\"_blank\">" + GXml.value(name[0]) + "</a></strong>";
		    } else {
        		html += "<strong>" + GXml.value(name[0]) + "</strong>";
				}
    }
    ...
    if (civicNumber.length == 1) {
        html += "<em>" + GXml.value(civicNumber[0]) + ",&nbsp;</em>";
    }
    ...
    if (streetAddress.length == 1) {
        html += "<em>" + GXml.value(streetAddress[0]) + ",&nbsp;</em>";
    }
    ...
    if (province.length == 1) {
        html += "<em>" + GXml.value(province[0]) + ",&nbsp;</em>";
    }
    ...
    if (postalCode.length == 1) {
        html += "<em>" + GXml.value(postalCode[0]) + ",&nbsp;</em>";
    }
    ...
    if (country.length == 1) {
        html += "<em>" + GXml.value(country[0]) + "</em>";
    }
    ...
    if (phone.length == 1) {
        html += "<em>" + GXml.value(phone[0]) + "</em><br />";
    }
    ...
    if (transit.length == 1) {
        html += "<strong>" + GXml.value(transit[0]) + "</strong><br />";
    }
...
HotspotsMap.prototype.parseHotspotsStatus = function(xml_doc)
{
    ...
    for (var i = 0; i < hotspots.length; i++) {
        ...
        if (hotspotId.length == 1 && gis.length == 1 && gis[0].getAttribute("lat") != "" && gis[0].getAttribute("long") != "") {
            ...
            html_list += html + "<a href=\"javascript:" + this.external_object_name +".openInfoBubble('" + GXml.value(hotspotId[0]) + "');\" class=\"showme\">" + this.translations.show_on_map + "</a>";
